...
### 新增
- **首次启动引导**：用“发现能力、安装到 Agent、开始使用”的三步说明帮助新用户理解 PilotHub。
- **快速安装**：推荐能力支持自动检测已安装 Agent、选择可用目标并直接进入安装流程。
- **安装成功指引**：安装完成后展示目标 Agent、项目位置、使用提示和下一步操作。
- **能力卡片**：推荐 Skill 使用面向场景的中文名称、用途、分类、来源和适用平台，降低理解门槛。
- **安装诊断**：检查 GitHub 连接、Agent 检测、目标目录可写性和 Skill 格式，并提供可执行的失败建议。
- **本地匿名反馈**：可选记录安装开始、成功或失败及标准化环境类型；默认关闭，仅保存在本机，可随时清除。

### 变更
- **Alpha 能力边界冻结**：本版本专注第一次安装到成功使用的体验，不扩展 MCP、Agent、Prompt、Hook、工作流、市场或云端账号能力。
- **隐私边界明确**：匿名反馈不记录仓库 URL、Skill 名称、本地路径、用户内容、项目代码或原始错误，也不包含上传逻辑。

### 验证
- **完整质量检查**：前端 lint、单元测试和生产构建，以及 Rust 格式、Clippy 和单元测试均通过。
- **核心链路验收**：`baoyu-cover-image` 可由 PilotHub 安装并同步到 Antigravity 与 Codex，Extensions 页面可显示集合健康状态和 Agent 目标。
